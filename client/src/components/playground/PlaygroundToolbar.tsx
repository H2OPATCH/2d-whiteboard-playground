import { 
  FileText, 
  Image, 
  StickyNote, 
  Square, 
  Link2,
  Type,
  Video,
  File,
  GitBranch
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from "@/components/ui/tooltip";
import { cn } from "@/lib/utils";
import { Separator } from "@/components/ui/separator";

interface PlaygroundToolbarProps {
  activeTool: string | null;
  onSelectTool: (tool: string | null) => void;
}

export default function PlaygroundToolbar({ activeTool, onSelectTool }: PlaygroundToolbarProps) {
  const basicTools = [
    { id: 'text', icon: Type, label: 'Text' },
    { id: 'sticky', icon: StickyNote, label: 'Sticky Note' },
    { id: 'shape', icon: Square, label: 'Shape' },
  ];

  const mediaTools = [
    { id: 'image', icon: Image, label: 'Image' },
    { id: 'video', icon: Video, label: 'Video' },
    { id: 'pdf', icon: File, label: 'PDF' },
  ];

  const diagramTools = [
    { id: 'connector', icon: Link2, label: 'Connector' },
    { id: 'flowchart', icon: GitBranch, label: 'Flowchart' },
  ];

  const renderToolGroup = (tools: typeof basicTools) => (
    <>
      {tools.map((tool) => (
        <Tooltip key={tool.id}>
          <TooltipTrigger asChild>
            <Button
              variant="ghost"
              className={cn(
                "flex items-center p-2 rounded whitespace-nowrap",
                activeTool === tool.id && "bg-gray-200"
              )}
              onClick={() => onSelectTool(activeTool === tool.id ? null : tool.id)}
            >
              <tool.icon className="h-5 w-5 mr-2" />
              <span>{tool.label}</span>
            </Button>
          </TooltipTrigger>
          <TooltipContent>
            <p>{tool.label}</p>
          </TooltipContent>
        </Tooltip>
      ))}
    </>
  );

  return (
    <div className="bg-white border-b border-gray-200 p-3 flex space-x-2 overflow-x-auto">
      <TooltipProvider>
        {renderToolGroup(basicTools)}
        <Separator orientation="vertical" className="h-10" />
        {renderToolGroup(mediaTools)}
        <Separator orientation="vertical" className="h-10" />
        {renderToolGroup(diagramTools)}
      </TooltipProvider>
    </div>
  );
}